...
# Convert a SPIR-V .spv file to a C header with a uint32_t array and a length.
...
def emit_header(spv_path, hdr_path, sym_prefix):
    if not os.path.isfile(spv_path):
        raise FileNotFoundError(f"SPIR-V input not found: {spv_path}")

    with open(spv_path, "rb") as f:
        data = f.read()

    if len(data) % 4 != 0:
        raise ValueError(f"SPIR-V file length not multiple of 4: {spv_path}")
    ...
    with open(hdr_path, "w", newline="\n") as h:
        ...
        # emit 8 words per line for readability
...
def main():
    if len(sys.argv) != 4:
        print("Usage: python3 scripts/spv_to_header.py input.spv out_header.h symbol_prefix", file=sys.stderr)
        sys.exit(2)
    spv_path = sys.argv[1]
    hdr_path = sys.argv[2]
    sym_prefix = sys.argv[3]
    try:
        emit_header(spv_path, hdr_path, sym_prefix)
