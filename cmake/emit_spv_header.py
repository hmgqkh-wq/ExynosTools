...
if not os.path.isfile(in_spv):
    print(f"ERROR: Input SPV not found: {in_spv}", file=sys.stderr)
    sys.exit(3)
data = open(in_spv, "rb").read()
if len(data) == 0:
    print(f"ERROR: Input SPV empty: {in_spv}", file=sys.stderr)
    sys.exit(4)
pad = (4 - (len(data) % 4)) % 4
...
with open(out_h, "w", newline="\n") as fh:
    ...
    for i in range(0, len(words), 8):
        fh.write("    " + ", ".join("0x%08xu" % w for w in words[i:i+8]))
