...
XenoBCContext* xeno_bc_create_context(VkDevice device, VkPhysicalDevice phys) {
    ...
    VkDescriptorPoolSize poolSizes[2] = {{VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, 16}, {VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, 16}};
    ...
    // Create pipelines for all formats with ray tracing option
    const unsigned char* shaders[XENO_BC_FORMAT_COUNT] = {bc1_shader_spv, bc3_shader_spv, bc4_shader_spv, bc5_shader_spv, bc6h_shader_spv, bc7_shader_spv};
    ...
    ctx->initialized = 1;
    XENO_LOGI("bc_emulate: enhanced context created with ray tracing and optimizations for Xclipse 940");
    return ctx;
...
VkResult xeno_bc_decode_image(VkCommandBuffer cmd, XenoBCContext* ctx, VkBuffer src_bc, VkImage dst_rgba,
    ...
    result = multi_threaded_dispatch(cmd, ctx->pipelines[format], ctx->pipelineLayout, /*desc_set*/, extent, ctx->workgroup_size, 4, scale);
    if (result != VK_SUCCESS) {
    ...
    for (uint32_t mip = subres.baseMipLevel; mip < subres.baseMipLevel + subres.mipLevelCount; mip++) {
        for (uint32_t layer = subres.baseArrayLayer; layer < subres.baseArrayLayer + subres.arrayLayerCount; layer++) {
            VkImageMemoryBarrier preBarrier = {
                ...
                .image = dst_rgba,
                .subresourceRange = {.aspectMask = VK_IMAGE_ASPECT_COLOR_BIT, .baseMipLevel = mip, .levelCount = 1, .baseArrayLayer = layer, .layerCount = 1}
            };
            vkCmdPipelineBarrier(cmd, VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, 0, 0, NULL, 0, NULL, 1, &preBarrier);
            
            // Dispatch (already multi-threaded)
            ...
            vkCmdPipelineBarrier(cmd, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT, 0, 0, NULL, 0, NULL, 1, &postBarrier);
        }
    }
    
    XENO_LOGD("bc_emulate: decoded %dx%d texture (mip %u, layers %u) using format %d with scale %.2f", extent.width, extent.height, subres.mipLevelCount, subres.arrayLayerCount, format, scale);
    return VK_SUCCESS;
}
